...
% Создание tabpanel у которой можно выставить свой цвет backgroundColor,
% используя лишь стандартные средства Matlab для совместимости со старыми
% версиями программы
% Использовал код, который был представлен пользователем _Ivana здесь http://www.cyberforum.ru/matlab/thread1237853.html
% Цитата (_Ivana): "Много косяков, какие-то легко исправимые (например, отрицательная длина/ширина при ресайзе дает ошибку), какие-то пока 
% не знаю как исправить (чтобы коллбэк ресайза основного окна можно было написать отдельно и он не заменился моей функцией
% для ресайза панелей закладок). Параметры шрифта по умолчанию надо бы брать из системных установок а не в лоб задавать. 
% Здоровая конструктивная критика и полезные советы приветствуются :)"
% Пример:
% clear all; close all
...
%         ih = ishghandle(pr.parent)
        ...
        FontName = any2cell(pr.FontName, 'Tahoma');
        ...
        hpg = uipanel('Tag','ui_EPR_TabGroup','Parent',pr.parent,'units','pixels',....
                    'position',[0 0 1 pr.tabheight],'bordertype','none','backgroundColor',parentColor);
        hpanel = zeros(1, numtabs);
        htab = zeros(1, numtabs);
        bordpanel = uipanel('Parent',pr.parent,'units','pixels','bordertype','beveledout','borderwidth',1,'HighlightColor',parentColor*kColor);
        tableftpos = 0;
        for ii = 1:numtabs
            ...
            hp = uipanel('Parent',hpg,'units','Normalized','position',[tableftpos, 0, tabwidth{ii}, .85],....
                'backgroundColor',get(hpanel(ii),'backgroundColor'),'bordertype','etchedout','borderwidth',1,'HighlightColor',parentColor*kColor);
            htab(ii) = uicontrol('style','text','Parent',hp,'units','Normalized','position',[0 0 1 1],....
                'backgroundColor',kColor.*get(hp,'backgroundColor'),'string',pr.tabnames{ii},....
    ...
    function tabpanelPushTabFcn(h,~)
        ...
        set(tp,'backgroundColor',0.9.*get(pp,'backgroundColor'),'position',[0 0 1 1],'fontweight','normal','enable','inactive');
        pos = get(pp,'position');
        set(pp,'position',[pos(1:3), .85],'bordertype','etchedout','borderwidth',1,'HighlightColor',parentColor*kColor);%,'HighlightColor','w');
        
        set(tc,'backgroundColor',get(pc,'backgroundColor'),'position',[0 -.1 1 1.1],'fontweight','bold','enable','on');
        pos = get(pc,'position');
        set(pc,'position',[pos(1:3), 1],'bordertype','beveledout','borderwidth',1,'HighlightColor',parentColor*kColor);%,'HighlightColor','r');
