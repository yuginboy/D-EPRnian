function mDisplayEPRfiles_loadData_v_01(B)
% загружаем данные
...
% Если изменения произошли в списке Dir
...
if B.indexDir ~= B.indexDirOld
    ...
    set(B.hb_fixScale, 'String', 'Autosclae is On');
    B.freezeScale = get(B.hb_fixScale, 'Value');
    if ~strcmp (B.dirNameList{B.indexDir},'.') 
        if ~strcmp (B.dirNameList{B.indexDir},'.')
...
%             set(B.lb_text, 'style', 'listbox');
            ...
            set(B.hb_fixScale, 'String', 'Autosclae is Off');
            B.freezeScale = get(B.hb_fixScale, 'Value');
        end
    end
...
% Если изменения произошли в списке Files
...
% Если изменения произошли в списке Struct
