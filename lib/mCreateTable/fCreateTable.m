function [mtable, buttons] = fCreateTable(varargin)
...
%                        - 'issorter'=> 'on','off',1,0,true,false (default='on')
%
% Output parameters:
...
% Modified by yugin for DEPRnian project $Date: 2015/09/02 21:22:00 $
  ...
  %try
      ...
      if ~isempty(which('TableSorter')) && paramsStruct.issorter
          ...
          jtable.getTableHeader.setToolTipText('<html>&nbsp;<u><b>main Curve Table:</b></u> <br> &nbsp;<b>Click</b> to sort up; <b>Shift-click</b> to sort down<br>&nbsp;<b>Ctrl-click</b> (or <b>Ctrl-Shift-click</b>) to sort secondary&nbsp;<br>&nbsp;<b>Click again</b> to change sort direction<br>&nbsp;<b>Click a third time</b> to return to unsorted view<br>&nbsp;<b>Right-click</b> to select entire column</html>');
      else
          % Set the header tooltip (no sorting instructions...)
          jtable.getTableHeader.setToolTipText('<html>&nbsp;<u><b>Region Table:</b> <br></u> &nbsp;<b>Double Click</b> to select parameters for viewing in the main <b>Curve</b> table<br>&nbsp;&nbsp;<b>Curve Name</b> or <b>Angle</b> to change second column values in the main <b>Curve</b> table&nbsp;<br>&nbsp;&nbsp;<b>Other</b> parameters to show like peak/region <b>number</b> and parameters name&nbsp;</html>');
      end
...
function paramsStruct = processArgs(varargin)
    ...
    try
        ...
        supportedArgs = {'container','headers','data','buttons','imagecolumns','imagetooltipheight','issorter'};
        while ~isempty(pvPairs)

            % Ensure basic format is valid
            paramName = '';
            if ~ischar(pvPairs{1})
                error('DEPRnian:createTable:invalidProperty','Invalid property passed to createTable');
            elseif length(pvPairs) == 1
                error('DEPRnian:createTable:noPropertyValue',['No value specified for property ''' pvPairs{1} '''']);
            end
        ...
        if ischar(paramsStruct.buttons)
            switch lower(paramsStruct.buttons)
                ...
                otherwise
                    error('DEPRnian:createTable:invalidProperty','Invalid buttons property value: must be ''on'', ''off'', 1, 0, true or false');
            end
        elseif isempty(paramsStruct.buttons) || ~(isnumeric(paramsStruct.buttons) || islogical(paramsStruct.buttons))
            error('DEPRnian:createTable:invalidProperty','Invalid buttons property value: must be ''on'', ''off'', 1, 0, true or false');
        end
        
        if ischar(paramsStruct.issorter)
            switch lower(paramsStruct.issorter)
                case 'on',  paramsStruct.issorter = true;
                case 'off', paramsStruct.issorter = false;
                otherwise
                    error('DEPRnian:createTable:invalidProperty','Invalid issorter property value: must be ''on'', ''off'', 1, 0, true or false');
            end
        elseif isempty(paramsStruct.issorter) || ~(isnumeric(paramsStruct.issorter) || islogical(paramsStruct.issorter))
            error('DEPRnian:createTable:invalidProperty','Invalid issorter property value: must be ''on'', ''off'', 1, 0, true or false');
        end
    catch
        if ~isempty(paramName),  paramName = [' ''' paramName ''''];  end
        error('DEPRnian:createTable:invalidProperty',['Error setting createTable property' paramName ':' char(10) lasterr]);
    end
%end  % processArgs
...
function processParams(paramsStruct,mtable,jtable)
    try
        ...
        elseif ~isempty(paramsStruct.imagecolumns)  % i.e., missing Renderer
            warning('DEPRnian:createTable:missingJavaClass','Cannot set image columns: ImageCellRenderer.class is missing from the Java class path');
        end
        jtable.repaint;
