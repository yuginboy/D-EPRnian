function f_CreatePeakInTheRegion (B, masterRegion)
% Функция поиска ЭПР пиков в области, которая передается в переменной
% masterRegion.
% Предназначена для создания и пропагирования(копирования) свойств-регионов
% на выделенный набор кривых.
axes(B.hAxes);
hold on;
cn = 1;
...
RegionSearchCoord = masterRegion.RegionSearchCoord;
...
% Если копирование происходит с совершенно непохожего спектра, где регион
% поиска пика выходит за рамки самого спектра, то преобразуем регион поиска
% в относительные единицы и применим его к данному спектру.
if length(ii) < 3
    RegionSearchCoord = masterRegion.RegionSearchCoordRelative * abs(B.x(end) - B.x(1))  +  B.x(1);
    disp('Master region is outside of the spectrum. Create region from the relative coordinats.');
end


Y2 = max(max(B.y(ii)));
x1 = RegionSearchCoord(1);
x2 = RegionSearchCoord(2);
if x2 < x1
    tmpVar = x2;
    x2 = x1;
    x1 = tmpVar;
